...
*Setup instructions for a gitian build of healthheldtoken using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the healthheldtoken
...
- [Building healthheldtoken](#building-healthheldtoken)
...
Clone the git repositories for healthheldtoken and gitian and then checkout the healthheldtoken version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/healthheldtoken-project/healthheldtoken.git
cd healthheldtoken
...
Follow the instructions in [doc/release-process.md](release-process.md) in the healthheldtoken repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building healthheldtoken
----------------

To build healthheldtoken (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the healthheldtoken repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/healthheldtoken/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/healthheldtoken-project/healthheldtoken
...
For example:
```bash
URL=https://github.com/user/healthheldtoken.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit healthheldtoken=${COMMIT} --url healthheldtoken=${URL} ../healthheldtoken/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit healthheldtoken=${COMMIT} --url healthheldtoken=${URL} ../healthheldtoken/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit healthheldtoken=${COMMIT} --url healthheldtoken=${URL} ../healthheldtoken/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/healthheldtoken-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/healthheldtoken-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/healthheldtoken-build.assert
```
...
[healthheldtoken/gitian.sigs](https://github.com/healthheldtoken-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
