...
Value ValueFromAmount(const CAmount& amount, int ver)
{
    //TODO: Need update in the future
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop healthheldtoken server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "healthheldtoken server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use healthheldtokend, or the -server option to healthheldtoken-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> healthheldtoken-cli " + methodname + " " + args + "\n";
}
